...
const second_message=`your zodiac sign is${zodiacSigns[Month-1]}`;//month ke hisab se
const third_message=compliments[Day-1];
    let index = Math.floor(Math.random()*20);
    const fourth_message =  victimCardCompliments[index];

    index = (Name.length * SurName.length * Year)%30; //*user ke liye fixed rehna chhaiye
    const fifth_message = recommendations[index];

     index = (Day*Month*Year)%20;
    const sixth_message = predictions[index]; //fixed rahega


    result.innerHTML = `
        <b>${first_message}</b><br/>
        ${second_message}<br/>
        <i>${third_message}</i><br/>
        <span style="color:#ffc107;">${fourth_message}</span><br/>
        <b>Our Recommendation for you:</b> ${fifth_message}<br/>
        <b>Your Future Prediction is:</b> ${sixth_message}
    `;
});
